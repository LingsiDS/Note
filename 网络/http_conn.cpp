...
//网站的根目录
const char *doc_root = "/home/lingsi/web";
...
void http_conn::init(int sockfd, const sockaddr_in& addr) {
	m_sockfd = sockfd;
	m_address = addr;
	//一下两行避免TIME_WAIT状态，仅用于调试，实际使用应去掉
	int reuse = 1;
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text) {
	//遇到空行，表示头部字段解析完毕
	if (text[0] == '\0') {
		//如果HTTP请求有消息体，则还需要读取m_content_length字节的消息，状态转移到CHECK_STATE_CONTENT状态
...
//我们并没有真正解析HTTP请求的消息体，只是判断它是否被完整地读入了
http_conn::HTTP_CODE http_conn::parse_content(char *text) {
	if (m_read_idx >= (m_content_length + m_checked_idx)) {//消息体已经读入
	...
	while ( ((m_check_state == CHECK_STATE_CONTENT) && (line_status == LINE_OK)) ||
		...
		switch(m_check_state) {
			...
			case CHECK_STATE_CONTENT: {
				...
				if (ret == GET_REQUEST) {//消息体已经读入
					return do_request();
				}
				line_status = LINE_OPEN; //消息体还没读入
				break;
			}
			default: {
...
void http_conn::process() {
	...
	modfd(m_epollfd, m_sockfd, EPOLLOUT);//m_sockfd可写事件就绪，主线程监听该事件
}